  ...
  async function handleAddUser() {
    ...
    try {
      await apiPost('/api/users', {
        ...
        isPlatformAdmin: newUserData.isPlatformAdmin,
  ...
  async function handleEditUser() {
    ...
    try {
      await apiPatch(`/api/users/${editingUser.id}`, {
        ...
        isPlatformAdmin: editFormData.isPlatformAdmin,
          ...
          <div className="flex-1 overflow-y-auto space-y-2 py-1">
            ...
            <div className="space-y-2 border-t border-border pt-3">
              ...
              <label className="flex items-start gap-2 rounded-md border border-border p-3 text-xs">
                <input
                  type="checkbox"
                  checked={newUserData.isPlatformAdmin}
                  onChange={(event) => setNewUserData((prev) => ({ ...prev, isPlatformAdmin: event.target.checked }))}
                  className="mt-0.5"
                />
                <span><span className="block font-medium text-foreground">平台管理员</span><span className="text-muted-foreground">可访问平台后台并管理所有团队，仅授予受信任的运维账号。</span></span>
              </label>
            </div>
            <div className="space-y-1">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 py-4">
            ...
            <div className="space-y-3 border-t border-border pt-4">
              ...
              <label className="flex items-start gap-2 rounded-md border border-border p-3 text-sm">
                <input
                  type="checkbox"
                  checked={editFormData.isPlatformAdmin}
                  disabled={loggedInUser?.id === editingUser?.id}
                  onChange={(event) => setEditFormData((prev) => ({ ...prev, isPlatformAdmin: event.target.checked }))}
                  className="mt-0.5"
                />
                <span><span className="block font-medium text-foreground">平台管理员</span><span className="text-muted-foreground">可访问平台后台并管理所有团队。</span></span>
              </label>
            </div>
          </div>
          <DialogFooter>
