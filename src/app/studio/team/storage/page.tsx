...
type Data = { quota: { maxStorageGB: number }; usage: { usedBytes: string; recycleBinBytes: string; bySource: { videos: string; assets: string; uploads: string; photos: string }; projects: number; members: number }; projects: Array<{ id: string; title: string; sizeBytes: string; updatedAt: string }> }
const formatBytes = (value: string | number) => { const bytes = Number(value); if (!bytes) return '0 B'; const units = ['B', 'KB', 'MB', 'GB', 'TB']; const i = Math.min(Math.floor(Math.log(bytes) / Math.log(1024)), units.length - 1); const amount = bytes / 1024 ** i; return `${amount >= 10 || i === 0 ? amount.toFixed(0) : amount.toFixed(2)} ${units[i]}` }
